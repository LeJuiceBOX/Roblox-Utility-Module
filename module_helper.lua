...
local module = {}
	...
	module.PluginPrint = function(cont,t)
		local text = "[ RUM ] - "..tostring(cont)  
	...
	module.DebugPrint = function(cont,t)
		if script.Parent["[ DebugMode ]"].Value then module.PluginPrint(cont,t); end
	end
	
	module.HasProp = function(object,property)
		if object:FindFirstChild(property) then 
			module.PluginPrint("A child cant have the same name as one of its parent's properties. (its also bad practice)",'warn')
			return; 
		end
